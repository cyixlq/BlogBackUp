...
### 一，打这个补丁有什么用处？
打这个补丁就能成功驱动你的`核显`，让它正常工作。如果已经成功驱动了核显的就没必要看了。另外本文不适用与双显卡用户，双显卡一些配置与此处有出入，还望绕道。
### 二，准备工作
* 添加启动参数 -cdfon（不加也OK，这个），删除启动参数 -disablegfxfirmware
...
2. framebuffer-stolenmem（翻译成中文为丢失的内存，猜想：给BIOS中DVMT添加内存大小。会影响高分屏，这个值加上BISO中设置的DVMT的大小必须大于32M，也不应该过高）：
- 一般1080P屏幕的话，和大于32M就够用了：00003001
...
11. model（应该是显示在关于本机中的显卡型号）：
* 字符串(STRING)类型，取值：Intel HD Graphics XXXX（XXXX为你的核显型号）
